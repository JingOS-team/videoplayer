...
Item {
    ...
    Flow {
        ...
        anchors {
            fill: parent
            margins: units.smallSpacing* 5
        }

        readonly property real cellSizeHint: columnWidth // units.iconSizes.large + units.smallSpacing * 5
        readonly property real columnWidth: parent.width / 10 //Math.floor(width / Math.floor(width / cellSizeHint))
        spacing: parent.width / 10 //(parent.width - columnWidth * 5) / 5
